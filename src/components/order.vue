  ...
  <div class="order">
  	...
  	<div class="pose">
  		<p @click="selectType(3)">
  			...
  			<span :class="{addr_text:sendType == 3}">到店自提  </span>
  		</p>
  		<p @click="selectType(1)">
  			<span v-if="sendType == 1" class="iconfont icon-yuanquanxuanzhong"></span>
  			<span v-else class="iconfont icon-yuanquanweixuan"></span>
  			<span :class="{addr_text:sendType == 1}">商家配送</span>
  		</p>
  		<p @click="selectType(4)">
  			<span v-if="sendType == 4" class="iconfont icon-yuanquanxuanzhong"></span>
  			<span v-else class="iconfont icon-yuanquanweixuan"></span>
  			<span :class="{addr_text:sendType == 4}">到店付款</span>
  		</p>
  	</div>
  	<!--到店自提-->
  	...
  	<!--到店付款-->
  	<div class="pay_store" v-if="address && sendType == 4">
  		店铺地址   : <span>{{address}}</span>
  	</div>
  	...
  	<div class="list">
	  	<div class="order_list" v-for="(item,index) in orderList" :key="index">
	  		<div class="img">
	  			<img v-if="contentHttp(item.mainImg)" v-lazy="item.mainImg" alt="imgs" />
	  			<img v-else v-lazy="$baseUrl2 + item.mainImg" alt="imgs" />
	  		</div>
	  		<div class="detail">
	  			<p class="detail_name">{{item.name}}</p>
	  			<p class="detail_dec" v-if="item.specs && item.specs[0]">规格:{{item.specs[0].specName}}</p>
	  			<div class="detail_info">
	  				<p> 
	  					<span class="text">数量</span>
	  					<span class="text_1">×{{item.amount}}</span>
	  				</p>
	  				<p>
	  					<span class="text">单价</span>
	  					<span class="text_1" v-if="item.price">¥{{item.price}}</span>
	  					<span class="text_1" v-if="item.specs && item.specs[0]">¥{{item.specs[0].price}}</span>
	  				</p>
	  			</div>
	  		</div>
  		</div>
  	...
  	<div class="total_info">
  		<p>
  			...
  			<span class='text_3'>×{{countAll}}</span>
  		</p>
  		<p>
  	...
  	<div class="footer">
  		<div class="left">
  			<p class="text_2">合计:</p>
  			<p class="text_3">¥{{parseFloat(totailPrice).toFixed(2)}}</p>
  		</div>
...
export default {
  ...
  methods:{
  	...
  	getOrder(){
  		...
  		this.$http.get(this.$domain + "/goods/category?categoryId=" + this.categoryId).then(function(response){
      	if(response.data.result.return_code == "SUCCESS"){
      		_this.orderList = response.data.list;
      		_this.address = _this.orderList[0].selfAddress
      		if(_this.$cookie.get("address")){
			  		_this.poseInfo = _this.$cookie.get("address") + _this.address
      		...
      		if(_this.orderList && _this.orderList.length > 0){
      			_this.orderList.forEach(function(e){
      				var item = {};
      				_this.countAll += e.amount
					_this.seller = e.officeId
      				if(e.specs && e.specs.length > 0){
      					_this.totailPrice += e.specs[0].price * e.amount
      					item = {
      						"productId": e.specs[0].productId,
			            "specId": e.specs[0].id,
			            "amount": e.amount
      					}
      					_this.orderProduct.push(item)
      				}else{
      					_this.totailPrice += e.price* e.amount
      					item = {
      						"productId": e.id,
			            "amount": e.amount
      					}
  	...
  	//添加地址
  	addAddress(){
  		this.$router.push({path:"address",query:{"id":this.expressId,"categoryId":this.$route.query.categoryId,"userId":this.$route.query.userId}})
  	},
  	//选择取货方式
  	selectType(type){
  		this.sendType = type
  		if(this.sendType == 3){
  			this.poseInfo = this.$cookie.get("address") + this.address
  		}
  	},
  	//添加自提信息
  	addInfo(){
  	...
  	sure(){
  		...
  		if(this.phone && this.name){
  			this.poseInfo = this.name + "|" + this.phone + "|" + this.address
